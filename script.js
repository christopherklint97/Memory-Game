(function () {
  ...
  // Start and restart the game click events
  ...
  function handleCardClick(event) {
    ...
    // Records the clicks only if the same card is not clicked and there are less than two cards shown at a time
    if ((!clickedDiv.classList.contains('open')) && (!clickedDiv.classList.contains('face-up')) && ($('.open').length !== 2)) {
      counter++;

      // Increases the score on click
      increaseScore();
